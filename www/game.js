function ricarica(){ window.location.href = "./index.html";}
...
function paint()
{
    if(giocando)
    {
        ...
        if(nx < 0 || nx + cw/(2*cw)> canvasW/cw || ny <0 || ny > canvasH/cw || check_collision(nx, ny, snake_array))
        {
            salva(score);
            //restart game
            init();
...
{
    storage.removeItem('lastScore');
    storage.setItem('lastScore', score);
    if(score>storage.getItem('record'))
        {
            storage.removeItem('record'); // Pass a key name to remove that key from storage.
            storage.setItem('record', score); // Pass a key name and its value to add or update that key.
        }
}
